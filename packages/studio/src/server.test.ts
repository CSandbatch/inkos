...
describe("fair-play Studio API", () => {
  it("keeps sealed solutions off ordinary endpoints", async () => {
    ...
    expect((await app.request(`/api/v1/books/${bookId}/mystery/solution`, { method: "PUT", headers: { "content-type": "application/json", "x-novelgraph-capability": "solution:write" }, body: JSON.stringify(solution) })).status).toBe(200);
  ...
  it("exposes structured resources, validation, and advisory prose analysis", async () => {
    ...
    const charter = await (await app.request(`/api/v1/books/${bookId}/charters`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ mainThrust: "Solve the shared-account case", readerPromise: ["A fair solution"], protagonist: { name: "Mara" }, centralConflict: "Evidence contradicts testimony" }) })).json() as { id: string };
    expect((await app.request(`/api/v1/charters/${charter.id}/resolve`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ approved: true, rationale: "Approved for API workflow test" }) })).status).toBe(200);
    ...
    const ready = await (await app.request(`/api/v1/jobs/${job.id}/ready`, { headers: { "x-novelgraph-capabilities": "canon:propose" } })).json() as { nodes: Array<{ id: string; key: string }> };
